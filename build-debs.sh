...
build_arch() {
    ...
    rm -f "$OUT/travel-net_0.2.29-1_${arch}.deb"
...
EOF
    ...
    dpkg-deb -Zgzip --root-owner-group --build pkg_deb "$OUT/travel-net_0.2.29-1_${arch}.deb"
    echo "=== $arch -> $OUT/travel-net_0.2.29-1_${arch}.deb ==="
}

# All deps installed by postinst via apt. No hard Depends.
build_arch armv7-unknown-linux-gnueabihf armhf "hostapd, dnsmasq, wpasupplicant, iw, nftables, ntpdate, wireguard-tools"
build_arch aarch64-unknown-linux-gnu arm64 "network-manager, wpasupplicant, iw, nftables, ntpdate, wireguard-tools"
build_arch x86_64-unknown-linux-gnu amd64 "network-manager, wpasupplicant, iw, nftables, ntpdate, wireguard-tools"
